...
for label in SBIC_CATEGORICAL_COLUMNS:
    ...
    label_specific_model = args.__dict__[f"{label}_model"]
    if "deberta" in label_specific_model.lower():
        ic("===Loading model as DeBERTa model.")
        model = DebertaV2ForSequenceClassification.from_pretrained(
            label_specific_model, task_labels_map=task_labels_map
        )
    elif "roberta" in label_specific_model.lower():
        ic("===Loading model as RoBERTa model.")
        model = RobertaForSequenceClassification.from_pretrained(
            label_specific_model, task_labels_map=task_labels_map
        )
    elif "bert" in label_specific_model.lower() and "roberta" not in label_specific_model.lower():
    ...
    for batch in tqdm(
        ...
        logits = model(batch["input_ids"], task_name=label, attention_mask=batch["attention_mask"])[
            0
        ]
...
prediction_column_labels = [
    f"prediction_{label}_{BASE_MODEL_NAME}-{args.model_identifier}"
    for label in SBIC_CATEGORICAL_COLUMNS
]
