...
train_positive = train_dataset.filter(
    lambda x: x["label"] == 1,
    num_proc=12,
    cache_file_name=f"{DATA_CACHE_PATH}/aae-hf-train_filtered-pos.arrow",
)
train_negative = train_dataset.filter(
    lambda x: x["label"] == 0,
    num_proc=12,
    cache_file_name=f"{DATA_CACHE_PATH}/aae-hf-train_filtered-neg.arrow",
)
train_interleave = interleave_datasets([train_positive, train_negative], seed=42)
...
training_args = TrainingArguments(
    logging_dir=f"./logs/{MODEL_NAME}-aae-classifier-interleaving-seed{SEED}-{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}",
    ...
    save_steps=9000,
    logging_steps=100,
    output_dir=f"intermediate/{MODEL_NAME}-aee-classifier-interleaving-seed{SEED}-checkpoints",
    fp16=True,
)
trainer = Trainer(
    model=model,
    ...
    train_dataset=train_interleave,
...
model_save_path = f"model/{MODEL_NAME}-aee-classifier-interleaving-seed{SEED}"
